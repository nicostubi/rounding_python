...
def execute_round_to_four_decimals_with_decimal(value_chf, expected, decimals, n):
    print('---------------------------------------------------')
    print(f'CAS DE TEST {n} | montant_chf: {value_chf} | decimals: {decimals} | expected: {expected}')
    print('---------------------------------------------------')
    rounded_to_4_decimal = round_to_four_decimals_with_decimal(value_chf)
...
for _ in range(iterations):
    result = execute_round_to_four_decimals_with_decimal(val, float(str(val)[0:6]), 4, test_case_id)
    if not result:
        nb_failed_tests += 1
